// detail: https://atcoder.jp/contests/arc117/submissions/37530971
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  LucasCombination lc(3);
  mint ans = 0;
  for (int i = 0; i < n; i++) {
    mint val = ch_to_int(cs[i]);
    ans += val * lc.query(n-1, i);
  }
  if (n % 2 == 0) {
    ans *= -1;
  }
  cout << int_to_ch(ans.val()) << endl;
}
