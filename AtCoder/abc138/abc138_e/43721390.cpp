// detail: https://atcoder.jp/contests/abc138/submissions/43721390
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  for (auto ch: t) {
    if (first_index[current_idx][ch - 'a'] == -1) {
      concat_cnt++;
      current_idx = first_index[0][ch - 'a'];
    } else {
      current_idx = first_index[current_idx][ch - 'a'];
    }
  }

  current_idx++; // to 0-indexed
