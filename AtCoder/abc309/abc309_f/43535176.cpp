// detail: https://atcoder.jp/contests/abc309/submissions/43535176
#include <bits/stdc++.h>
using namespace std;
...
void verify(vector<array<int, 3>> hwd) {
  map<int, set<int>> h_to_w;
  for (int i = 0; i < (int)hwd.size(); i++) {
    auto [h, w, d] = hwd[i];

    if (not h_to_w[h].empty()) {
      int max_w = *h_to_w[h].rbegin();
      assert(w <= max_w);
    }
