// detail: https://atcoder.jp/contests/abc208/submissions/23981856
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  vector<warshall_floyd<ll>> gs(n,warshall_floyd<ll>(n));
  range(i,0,m){
    int a,b,c;
    cin>>a>>b>>c;
    a--,b--;
  ...
  range(i,0,n){
    range(j,0,n){
      range(k,0,n){
        if(gs[i][j][k]==gs[0].inf()) continue;
        if(gs[i][j][k]==0) continue;
        // debug(i,j,k,gs[i][j][k]);
        ans+=gs[i][j][k];
      }
    }
  }
