// detail: https://atcoder.jp/contests/abc172/submissions/22919612
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  ll ans=0;
  range(i,0,n){
    ll v=suma.query(0,i+1);
    ll valid=-2;
    ll invalid=m+2;
    while(abs(valid-invalid)>1){
      ll mid=(valid+invalid)/2;
      if(sumb.query(0,mid)+v<=k) valid=mid;
      else invalid=mid;
    }
    if(valid==-1) valid=-INF64;
    ll ind=lower_bound(sumb.sum.begin(),sumb.sum.end(),k-v)-sumb.sum.begin();
    if(sumb.sum[ind]>k-v){
      //ind--;
    }
    if(ind<=0){
