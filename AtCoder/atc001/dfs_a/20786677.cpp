// detail: https://atcoder.jp/contests/atc001/submissions/20786677
#include <bits/stdc++.h>
using namespace std;
...
struct GridBFS{
  ...
  GridBFS(int h,int w):H(h),W(w),s(h,vector<char>(w)),dy({0,1,0,-1}),dx({1,0,-1,0}){}

  bool rangeCheck(int y,int x){ return 0<=y&&y<H&&0<=x&&x<W; }

  void setStart(int y,int x){ sy=y; sx=x; }

  void setGoal(int y,int x){ gy=y; gx=x; }
...
int main() {
  ...
  range(i,0,h) range(j,0,w){
    cin>>s[i][j];
    if(s[i][j]=='s') s.setStart(i,j);
    if(s[i][j]=='g') s.setGoal(i,j);
  }

  vector<vector<int>> arrived = s.build();
