// detail: https://atcoder.jp/contests/arc109/submissions/41253597
#include <bits/stdc++.h>
...
namespace tools_library {
  ...
  class ModInt {
    ...
    constexpr ll extgcd(ll a, ll b, ll &x, ll &y) const {
      if (b == 0) {
        x = 1;
        y = 0;
        return a;
...
char dfs(const string &s, const int &n, int l, int r) {
  if (r - l <= 1) {
    return s[l % n];
  }

  int m = (l + r) / 2;
  char win1 = dfs(s, n, l, m);
  char win2 = dfs(s, n, m, r);

  char win = winner(win1, win2);
  debug(win, win1, win2, l, r);

  return winner(win1, win2);
}


char naive(int n, int k, string s) {
  char ans = dfs(s, n, 0, 1 << k);
  return ans;
}
...
int main() {
  ...
  mint sum_a = mint(whole(accumulate, a, 0LL));

  mint r = mint(2).pow(k);

  mint sum1 = sum_a * r;
  sum1 = mint(sum1.val() / n);
  ...
  mint sump = 0;
  rep(i, k - 1) {
    sump += mint(2).pow(mint(2).pow(i).val());
  }

  mint ansv = (mint(2).pow(r.val()) * sum) - sump;
