// detail: https://atcoder.jp/contests/cf17-final/submissions/30393745
#include <bits/stdc++.h>
using namespace std;
...
bool is_valid_kih(string s) {
  int idx = s.find_first_of('K');
  return s[idx + 1] == 'I' and s[idx + 2] == 'H';
}

bool is_valid_between_kih_and_b(string s) {
  int idx_h = s.find_first_of('H');
  int idx_b = s.find_first_of('B');

  idx_h++;
  int cnt_A = idx_b - idx_h;
  return cnt_A <= 1;
}

bool is_valid_between_b_and_r(string s) {
  int idx_b = s.find_first_of('B');
  int idx_r = s.find_first_of('R');

  idx_b++;
  int cnt_A = idx_r - idx_b;
  return cnt_A <= 1;
}

bool is_valid_front_of_k(string s) {
  int idx_k = s.find_first_of('K');
  int cnt_A = idx_k;
  return cnt_A <= 1;
}

bool is_valid_back_of_r(string s) {
  int idx_r = s.find_first_of('R');
  idx_r++;
  int cnt_A = len(s) - idx_r;
  return cnt_A <= 1;
}
...
int main() {
  ...
  // KIH が連続である
  // KIH と B の間は A が一つ以下である
  // B と R の間は A が一つ以下である
  // K の前は A が一つ以下である
  // R の後は A が一つ以下である

  if (is_valid_kih(s)
  and is_valid_between_kih_and_b(s)
  and is_valid_between_b_and_r(s)
  and is_valid_front_of_k(s)
  and is_valid_back_of_r(s)) {
    cout << "YES" << endl;
  } else {
    cout << "NO" << endl;
  }
