// detail: https://atcoder.jp/contests/abc225/submissions/26910465
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  auto correct = make_vector(n, m, 0);
  correct[0][0] = bs[0][0];
  rep (j, 1, m) { correct[0][j] = correct[0][j - 1] + 1; }
  rep (i, 1, n) {
    correct[i][0] = correct[i - 1][0] + 7;
    rep (j, 1, m) { correct[i][j] = bs[i][j - 1] + 1; }
  }

  cout << (bs == correct ? "Yes" : "No") << endl;
}
