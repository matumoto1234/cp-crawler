// detail: https://atcoder.jp/contests/arc098/submissions/25841946
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  cumulative_sum<int> west(n),east(n);
  range(i,n){
    if(s[i]=='W') west.set(i,1);
    else east.set(i,1);
  }
  ...
  int ans=INF32;
  range(i,n){
    chmin(ans,west.query(0,i)+east.query(i+1,n));
  }
  cout<<ans<<endl;
}