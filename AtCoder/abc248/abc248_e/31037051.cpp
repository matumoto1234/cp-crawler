// detail: https://atcoder.jp/contests/abc248/submissions/31037051
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  vector<pii> input_ps(n);
  vector<Point> ps;
  ps.reserve(n);
  for (auto &ip: input_ps) {
    cin >> ip.first >> ip.second;
    Point p(ip.first, ip.second);
    ps.push_back(p);
  }
  ...
  rep(i, n) {
    rep(j, n) {
      ...
      lines[pii(dy, dx)] = Line(ps[i], ps[j]);
    }
  }

  int ans = 0;

  for (auto [tilt, line]: lines) {
    int intersect_cnt = 0;
