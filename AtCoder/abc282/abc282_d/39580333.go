// detail: https://atcoder.jp/contests/abc282/submissions/39580333
package main
...
func main() {
	...
	// 連結成分ごとの白頂点の個数
	whiteCountList := make([]int, len(CCList))
	for v, color := range colorList {
		...
		whiteCountList[ccIdx]++
	}

	ans := n * (n - 1) / 2
	for i := range CCList {
		blackCount := len(CCList[i]) - whiteCountList[i]
		whiteCount := whiteCountList[i]
		ans -= blackCount * (blackCount - 1) / 2 + whiteCount * (whiteCount - 1) / 2
	}
	ans -= m
