// detail: https://atcoder.jp/contests/typical90/submissions/23121324
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  int n;
  cin >> n;
  vector<vector<int>> ps(2,vector<int>(n));
  rep(i,n){
    int c,p;
    cin>>c>>p;
    c--;
  ...
  vector<CumulativeSum<int>> sums(2);
  rep(i,2){
    sums[i].build(ps[i]);
  }
  ...
  rep(i,q){
    ...
    cin>>l>>r;
    l--,r--;
    cout<<sums[0].query(l,r+1)<<" "<<sums[1].query(l,r+1)<<newl;
  }
}