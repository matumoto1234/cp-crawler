// detail: https://atcoder.jp/contests/abc205/submissions/23445057
#include <bits/stdc++.h>
using namespace std;
...
inline void put(int y,int x){ row[y]=col[x]=1; }
inline void rm(int y,int x){ row[y]=col[x]=0; }
inline bool isEmpty(int y,int x){ return !row[y]&&!col[x]; }
...
bool dfs(int idx){
  if(idx==n){
	...
	for(int y=a[idx];y<=c[idx];y++){
		for(int x=b[idx];x<=d[idx];x++){
