// detail: https://atcoder.jp/contests/abc026/submissions/23925293
...
int main() {
  ...
  auto f = [&](double t) {
    double radian = c * t * M_PI;
    return a * t + b * sin(radian);
  };


  constexpr int limit = 2e8;

  range(i, 0, limit) {
    double t = (double)i / 1e8;
    if ( abs(f(t) - 100.0) < 1e-6 ) {
      cout << t << endl;
      return 0;
    }
  }
