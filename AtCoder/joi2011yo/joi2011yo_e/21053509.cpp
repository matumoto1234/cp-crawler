// detail: https://atcoder.jp/contests/joi2011yo/submissions/21053509
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  dist[0]=gridBFS(s,'S',"X");
  range(i,1,n){
    char sc=i+'0';
    dist[i]=gridBFS(s,sc,"X");
  }

  ll ans=0;
  range(i,0,n) range(j,0,h) range(k,0,w){
    char nextc=(i+1)+'0';
    if(s[j][k]==nextc) ans+=dist[i][j][k];
  }
  cout<<ans<<endl;
}