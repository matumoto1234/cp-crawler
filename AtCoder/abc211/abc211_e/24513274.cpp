// detail: https://atcoder.jp/contests/abc211/submissions/24513274
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  do{
    ...
    if(used.find(memo)!=used.end()) continue;

    auto arrived=grid_bfs(ns,'@',"#.");
    bool is_valid=true;
    range(i,n){
      range(j,n){
        if(ns[i][j]=='@' && arrived[i][j]==-1){
          is_valid=false;
        }
      }
    }
