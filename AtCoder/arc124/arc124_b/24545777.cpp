// detail: https://atcoder.jp/contests/arc124/submissions/24545777
#include <bits/stdc++.h>
using namespace std;
...
vector<int> get_cnt(const vector<int> &xs){
  
  vector<int> cnt(comp.xs.size());
  range(i,xs.size()){
    cnt[comp.get(xs[i])]++;
  }
  return cnt;
...
int main() {
  ...
  range(i,n){
    range(j,n){
      int x=as[i]^bs[j];
      int idx=comp.get(x);
      if(cnt[idx]<n) continue;
      if(edges.find(to_v(j,x))!=edges.end()) continue;

      edges.emplace(to_v(j,x));
      in_degree[idx]++;
    }
  }
