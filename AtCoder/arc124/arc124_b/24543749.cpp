// detail: https://atcoder.jp/contests/arc124/submissions/24543749
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  range(i,n){
    ...
    range(j,n){
      ...
      // if(ab[x].find(a)!=ab[x].end() || ab[x].find(b)!=ab[x].end()){
      //   used.insert(x);
      //   continue;
      // }
      // ab[x].insert(a);
  ...
  mf_graph<ll> G(2*n+1+sinks.size());
  int src = 2*n;
  ...
  range(i,n){
    ...
    range(j,n){
      ...
      int x=a^b;
      if(xs[x]>=n){
        if(edges.find(P(i,n+j))==edges.end()){
          G.add_edge(i,n+j,1);
          edges.emplace(i,n+j);
        }
        if(edges.find(P(n+j,sinks[x]))==edges.end()){
          G.add_edge(n+j,sinks[x],1);
          edges.emplace(n+j,sinks[x]);
          in_degree[x]++;
        }
      }
    }
