// detail: https://atcoder.jp/contests/arc086/submissions/25157335
#include <bits/stdc++.h>
using namespace std;
...
bool solve1(int n,vector<int> as){
  vector<P> ans;
  rrange(i, 1, n) {
...
bool solve2(int n,vector<int> as){
  vector<P> ans;
  range(i, n - 1) {
    int &l = as[i], &r = as[i + 1];
...
int main() {
  ...
  if(solve1(n,as)) return 0;
  if(solve2(n,as)) return 0;
  assert(false);
}