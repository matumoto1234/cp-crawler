// detail: https://atcoder.jp/contests/abc181/submissions/23923277
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  bool ans=false;
  range(i,0,n) range(j,0,n) range(k,0,n){
    if(i==j || j==k || i==k) continue;
    auto [xi,yi]=xys[i];
    auto [xj,yj]=xys[j];
    auto [xk,yk]=xys[k];
    // if (abs(atan2(xi,yi)-atan2(xj,yj))<1e-9 && abs(atan2(xi,yi)-atan2(xk,yk))<1e-9){
    //   debug(xi,yi, xj,yj, xk,yk);
    //   ans=true;
    // }
    if((yi-yj)*(xj-xk)==(yj-yk)*(xi-xj)){
      ans=true;
    }
  }
