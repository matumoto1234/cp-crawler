// detail: https://atcoder.jp/contests/arc041/submissions/26539005
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  rep (i, len(vs)) {
    if (vs[i] == 0) {
      if (i + 1 == len(vs) or vs[i + 1]) continue;
      vs[i] = -1;
    }

    if (vs[i]) {
      if (i - 1 == -1 or vs[i - 1] == 0) continue;
      vs[i] = -1;
    }
  }
  ...
  for (auto it = xd_map.begin(); next(it) != xd_map.end(); it++) {
    auto [x, d] = *it;
    auto [nx, nd] = *next(it);

    // debug(x, d, nx, nd);

    if (d == 1 and nd == 0) ans += nx - x - 1;
  }
