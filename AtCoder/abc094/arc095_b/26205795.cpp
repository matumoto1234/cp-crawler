// detail: https://atcoder.jp/contests/abc094/submissions/26205795
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  whole(sort,as);
  int idx=lower_bound(as.begin(),as.end(),as.back()/2)-as.begin();

  ll ma=-INF64;
  int ans=-1;
  if(idx-1>=0 and chmax(ma,binomial<ll>(as.back(),as[idx-1]))){
    ans=idx-1;
  }

  if(chmax(ma,binomial<ll>(as.back(),as[idx]))){
    ans=idx;
  }

  if(idx+1<n and chmax(ma,binomial<ll>(as.back(),as[idx+1]))){
    ans=idx+1;
  }

  cout<<as.back()<<' '<<as[ans]<<endl;
}