// detail: https://atcoder.jp/contests/abc232/submissions/28002675
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  rrep(i, h) {
    rrep(j, w) {
      if (s[i][j] == '#') continue;
      if (i - 1 >= 0 and s[i - 1][j] != '#') G[to_v(h, i, j)].emplace_back(to_v(h, i - 1, j));
      if (j - 1 >= 0 and s[i][j - 1] != '#') G[to_v(h, i, j)].emplace_back(to_v(h, i, j - 1));
    }
  }

  queue<int> q;
  vector<bool> used(h * w);
  vector<int> depth(h * w, -INF32);

  rrep(i, h) {
    rrep(j, w) {
      if (s[i][j] == '#') continue;
      if (used[to_v(h, i, j)]) continue;
  ...
  rep(i, len(depth)) { chmax(ans, depth[i]); }
