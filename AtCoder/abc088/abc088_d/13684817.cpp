// detail: https://atcoder.jp/contests/abc088/submissions/13684817
#include<bits/stdc++.h>
using namespace std;
...
int dfs(int x, int y){
  ...
  if(flgStack[x][y]==true){
    ...
    int flgNode = 0;
    for (int i = 0; i < 4; i++){
      if(x+nextX[i]<0||y+nextY[i]<0||x+nextX[i]>=h||y+nextY[i]>=w){
        continue;
      }
      if(s[x+nextX[i]][y+nextY[i]]=='.' && done[x+nextX[i]][y+nextY[i]]==false){
        flgNode++;
      }
    }
    if(flgNode>=2){
