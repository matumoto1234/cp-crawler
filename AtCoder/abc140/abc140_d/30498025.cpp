// detail: https://atcoder.jp/contests/abc140/submissions/30498025
#include <bits/stdc++.h>
using namespace std;
...
int calc_happiness(string s) {
  int res = 0;
  rep(i, len(s)) {
    if (s[i] == 'L' and i - 1 >= 0 and s[i - 1] == 'L') {
      res++;
    }

    if (s[i] == 'R' and i + 1 < len(s) and s[i + 1] == 'R') {
      res++;
    }
  }
  return res;
...
int main() {
  ...
  rep(i, len(zipped)) {
    auto [ch, length] = zipped[i];
    if (chmax(max_len, length)) {
      max_len_char = ch;
      max_len_idx = i;
    }
  }
  ...
  rep(i, begin, 2 * k + begin, 2) {
    int idx1 = max_len_idx + i;
    int idx2 = max_len_idx - i;

    chmin(idx1, len(zipped) - 1);
    chmax(idx2, 0);

    // debug(idx1, idx2, i);

    auto &[ch1, length1] = zipped[idx1];
    auto &[ch2, length2] = zipped[idx2];
    ch1 = ch2 = max_len_char;
  }

  string restore = "";
  rep(i, len(zipped)) {
    const auto [ch, length] = zipped[i];
    restore += string(length, ch);
  }

  // debug(restore, zipped);


  int ans = calc_happiness(restore);
  cout << ans << endl;
}
