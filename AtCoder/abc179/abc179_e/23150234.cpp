// detail: https://atcoder.jp/contests/abc179/submissions/23150234
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  vector<Pll> used(m,Pll(-1,-1));
  vector<ll> a(m+10,0);

  ll loop_len=0,loop_sum=0;
  ...
  used[a[1]].first=1;
  used[a[1]].second=x;
  for(ll i=2;i<=min(n,m+1);i++){
    a[i]=(a[i-1]*a[i-1])%m;
    ll nlen=i,nsum=used[a[i-1]].second+a[i];
    auto &[len,sum]=used[a[i]];

    // debug(i,a[i],len,sum,nlen,nsum);
  ...
  debug(loop_sum,loop_len);

  if(loop_len>=1){
    ans+=n/loop_len*loop_sum;
    debug(n%loop_len);
    for(ll i=1;i<=n%loop_len;i++){
      ans+=a[i];
    }
  }
