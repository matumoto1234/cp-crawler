// detail: https://atcoder.jp/contests/abc235/submissions/39491855
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  vector<int> reachable(limit, false);
  queue<int> q;
  q.push(1);
  while (not q.empty()) {
    int x = q.front();
    ...
    auto dest1 = operation1(x, a);
    if ((0 <= dest1 and dest1 <= limit) and not reachable[dest1]) {
      graph.add_edge(WeightedEdge<long long>(x, dest1, 1));
      reachable[dest1] = true;
      q.push(dest1);
    }
    auto dest2 = operation2(x);
    if ((0 <= dest2 and dest2 <= limit) and not reachable[dest2]) {
      graph.add_edge(WeightedEdge<long long>(x, dest2, 1));
      reachable[dest2] = true;
      q.push(dest2);
    }
  }
