// detail: https://atcoder.jp/contests/arc125/submissions/25482576
#include <bits/stdc++.h>
using namespace std;
...
int find_left(vector<int> vs,int target,int idx=0){
  int n=len(vs);
  range(i,n) vs.emplace_back(vs[i]);
  range(i,idx,idx+n){
    if(vs[i]==target) return i%n;
  }
  return -1;
}

int find_right(vector<int> vs,int target,int idx=0){
  int n=len(vs);
  range(i,n) vs.emplace_back(vs[i]);
  rrange(i,idx,idx+n){
    if(vs[i]==target) return i%n;
...
int main() {
  ...
  set<int> sset,tset;
  range(i,n) sset.insert(s[i]);
  range(i,m) tset.insert(t[i]);
  if(sset!=tset){
    cout<<-1<<endl;
    return 0;
  }
  ...
  int idx=find_left(s,t[0]) < n-find_right(s,t[0]) ? find_left(s,t[0]) : find_right(s,t[0]);
  int ans=min(idx,n-idx)+vs[0].second;
  if(len(vs)>=2){
    idx=find_left(s,vs[1].first) < n-find_right(s,vs[1].first) ? find_left(s,vs[1].first) : find_right(s,vs[1].first);
    ans+=min(idx,n-idx)+vs[1].second;
  }
