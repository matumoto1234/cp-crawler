// detail: https://atcoder.jp/contests/arc125/submissions/25482359
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  int idx=min(find_left(s,t[0]),find_right(s,t[0]));
  int ans=idx+vs[0].second;
  if(len(vs)>=2){
    idx=min(find_left(s,vs[1].first),find_right(s,vs[1].first));
    ans+=idx+vs[1].second;
  }
