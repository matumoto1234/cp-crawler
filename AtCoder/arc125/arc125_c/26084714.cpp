// detail: https://atcoder.jp/contests/arc125/submissions/26084714
#include <bits/stdc++.h>
using namespace std;
...
int search(const vector<int> &as, int v){
  int valid=-1;
  int invalid=len(as);
  auto is_valid = [&](int m){
    return as[m]<v;
...
int search2(const vector<int> &as, int v){
  int valid=-1;
  int invalid=len(as);
  auto is_valid = [&](int m){
    return v<as[m];
...
int main() {
  ...
  exclude.reserve(n-k);
  range(i,1,n+1){
    if(aset.find(i)!=aset.end()) continue;
    exclude.emplace_back(i);
  }

  whole(sort,exclude);
  whole(reverse,exclude);
  cout<<as[0]<<' '<<exclude<<' ';
  as.erase(as.begin());
  cout<<as<<endl;
}