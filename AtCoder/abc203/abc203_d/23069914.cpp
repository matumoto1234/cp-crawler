// detail: https://atcoder.jp/contests/abc203/submissions/23069914
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  // map<ll,ll> mp;
  fenwick_tree<ll> ft(n*n+10);
  fenwick_tree<ll> preft(n*n+10);
  ...
  rep(i,n) rep(j,n){
    ...
    while(abs(valid-invalid)>1){
      ...
      // debug(valid,invalid,mid,ft.sum(0,mid));
    }
    // ll med=lower_bound(ft.dat.begin(),ft.dat.end(), k*k/2 + 1) - ft.dat.begin();
    ll med=valid;
    // debug(comp[med]);
    chmin(ans,comp[med]);
    ...
    if(gx+1<n){
      rep(l,k){
        ft.add(compa[i+l][j],-1);
        ft.add(compa[l+i][gx+1],1);
        // mp[a[i+l][gx+1]]++;
      }
    }
    // debug(i,j,gy,gx);
