// detail: https://atcoder.jp/contests/abc203/submissions/23073465
...
int main() {
  ...
  fenwick_tree<int> ft(n*n+10);
  fenwick_tree<int> preft(n*n+10);
  vector<fenwick_tree<int>> fts(2,fenwick_tree<int>(n*n+10));
  ...
  rep(i,n) rep(j,n){
    ...
    if(j==0){
      if(i==0){
        rep(l,k) rep(m,k){
          // ft.add(compa[l+i][m+j],1);
          fts[0].add(compa[l+i][m+j],1);
        }
        // preft=ft;
        fts[1]=fts[0];
      }else{
        rep(l,k){
          fts[i%2].add(compa[i-1][j+l],-1);
          fts[i%2].add(compa[gy][j+l],1);
          // preft.add(compa[i-1][j+l],-1);
          // preft.add(compa[gy][j+l],1);
        }
        // ft=preft;
      }
    }
    ...
    int invalid=-1;
    auto is_valid = [&](int m){
      return fts[i%2].sum(0,m)>=k*k/2 + (k*k%2);
    };
    ...
    if(gx+1<n){
      rep(l,k){
        fts[i%2].add(compa[i+l][j],-1);
        fts[i%2].add(compa[l+i][gx+1],1);
      }
    }
  }
  cout<<ans<<endl;
