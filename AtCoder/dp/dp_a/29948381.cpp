// detail: https://atcoder.jp/contests/dp/submissions/29948381
#include <bits/stdc++.h>
using namespace std;
...
vector<ll> dp(MAX_N + 1, INF32);
ll dfs(int idx) {
  if (idx == 0) {
    return 0;
  }
  if (idx == 1) {
    ll cost = abs(hs[1] - hs[0]);
    return cost + dfs(0);
  }
  ...
  ll cost1 = abs(hs[idx] - hs[idx - 1]);
  ll cost2 = abs(hs[idx] - hs[idx - 2]);
  ll v = min(cost1 + dfs(idx - 1), cost2 + dfs(idx - 2));
  chmin(res, v);
  return res;
}
