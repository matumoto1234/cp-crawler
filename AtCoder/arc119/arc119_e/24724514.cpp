// detail: https://atcoder.jp/contests/arc119/submissions/24724514
#include <bits/stdc++.h>
using namespace std;
using ll = long long;
...
ll swap_query(ll sum,ll i,ll j){
  if(!(0<=i<(int)as.size())) return INF;
  if(!(0<=j<(int)as.size())) return INF;
...
int main(){
  ...
  for(int i=0;i<(int)bs.size();i+=10){
    int idx=bs[i].second;
    for(ll j=i+1;j<(int)bs.size();j+=10){
      int idx2=bs[j].second;
      ans=min(ans,swap_query(sum,idx,idx2));
      ans=min(ans,swap_query(sum,idx,idx2+1));
      ans=min(ans,swap_query(sum,idx+1,idx2));
