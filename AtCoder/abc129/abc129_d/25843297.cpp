// detail: https://atcoder.jp/contests/abc129/submissions/25843297
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  auto cols=make_vector(h,w,-1);
  range(j,w){
    range(i,h){
      if(s[i][j]=='#') continue;
      if(cols[i][j]!=-1) continue;
      int cnt=0;
      range(k,i,h){
        if(s[k][j]=='#') break;
        cnt++;
