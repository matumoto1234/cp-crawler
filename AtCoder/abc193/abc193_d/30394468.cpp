// detail: https://atcoder.jp/contests/abc193/submissions/30394468
#include <bits/stdc++.h>
using namespace std;
...
map<T1, T2> merge_sum_map(map<T1, T2> a, map<T1, T2> b) {
  for (auto [key, val]: a) {
    b[key] += val;
  }
  return b;
...
ll number_of_all_case(int k, map<char, int> cnt_all) {
  ll all = 9 * k;
  for (auto [ch, cnt]: cnt_all) {
    all -= cnt;
  }

  return binomial(all, 2) * 2;
}


ll score(string s) {
  auto cnt = whole(counter, s);
  ll sum = 0;
  rep(i, 1, 10) {
    char c = i + '0';
    sum += i * power(10, cnt[c]);
  }
  return sum;
}
...
int main() {
  ...
  auto cnt_s = whole(counter, s);
  auto cnt_t = whole(counter, t);
  auto cnt_all = merge_sum_map(cnt_s, cnt_t);
  cnt_all['#'] = 0;
  ...
  rep(i, 1, 10) {
    ...
    rep(j, 1, 10) {
      ...
      cnt_all[cj]++;
      ll cj_case = k - cnt_all[cj] + 1;

      if (score(s) > score(t)) {
        // debug(s, t, ci, cj, cnt_all[ci], cnt_all[cj]);
        win_cnt += ci_case * cj_case;
      }
  ...
  ll all = number_of_all_case(k, cnt_all);

  // debug(win_cnt, all);
  cout << double(win_cnt) / all << endl;
}