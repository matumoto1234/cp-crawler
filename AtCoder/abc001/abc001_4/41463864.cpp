// detail: https://atcoder.jp/contests/abc001/submissions/41463864
#include <bits/stdc++.h>
using namespace std;
...
namespace data_structure_library {
  ...
  struct IntervalSet {
    ...
    bool isInf(pair<T, T> p) const {
      return p.first == inf() or p.first == -inf() or p.second == inf() or p.second == -inf();
    }
    ...
    bool insert(T x, T y) {
      ...
      if (!contains_x and !contains_y) {
        while (true) {
          auto it = prev(s.lower_bound(pair(y + 1, y + 1)));
          auto [l, u] = *it;
          if (x <= l and u <= y) {
            s.erase(it);
          } else {
        ...
        while (true) {
          auto it = prev(s.lower_bound(pair(yl, yu)));
          auto [l, u] = *it;
          if (x <= l and u <= yl) {
            s.erase(it);
          } else {
