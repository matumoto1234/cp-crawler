// detail: https://atcoder.jp/contests/abc001/submissions/41638037
...
namespace data_structure_library {
  ...
  struct IntervalSet {
    ...
    bool contains(T x) const {
      ...
      auto [l, r] = *prev(s.upper_bound(pair(x, x)));
      return l <= x and x < r;
    }

    // get { (l, r) | l <= x < r }
    pair<T, T> interval(T x) const {
      assert(contains(x));
      return *prev(s.upper_bound(pair(x, x)));
    }
    ...
    bool insert(T x) {
      ...
      return insert(pair(x, x));
    }

  private:
    typename set<pair<T, T>>::iterator find(int k) {
      return prev(s.upper_bound(pair(k, k)));
    }

    pair<T, T> extend_interval(pair<T, T> p1, pair<T, T> p2) {
    ...
    bool intersect(const pair<T, T> &p1, const pair<T, T> &p2) const {
      const auto &[a, b] = p1;
      const auto &[c, d] = p2;
      return (a <= c and c < b) or (c <= a and a < d);
    }

    bool continuous(const pair<T, T> &p1, const pair<T, T> &p2) const {
      const auto &[a, b] = p1;
      const auto &[c, d] = p2;
      return b == c;
    }
  ...
  public:
    ...
    // erase { (l, r) | l <= x < r }
    bool erase_interval(T x) {
      if (not contains(x)) {
        return false;
      }

      auto it = prev(s.upper_bound(pair(x + 1, x + 1)));
      s.erase(it);
      return true;
    }

    // { (l, r) | l <= x < r } => { (l, x), (x, r) | l <= x < r }
    bool cut(T x) {
      if (not contains(x)) {
        return false;
      }

      auto it = prev(s.upper_bound(pair(x, x)));
      auto [l, r] = *it;
      s.erase(it);
      s.emplace(l, x);
      s.emplace(x, r);
