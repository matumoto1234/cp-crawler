// detail: https://atcoder.jp/contests/abc307/submissions/42908144
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  for (int i = 0; i < (int)s.size(); i++) {
    ...
    if (s[i] == ')') {
      if (not st.empty() and st.top().first == '(') {
        int begin = st.top().second;
        st.pop();

        is.insert(pair(begin, i + 1));
        continue;
      }
  ...
  vector<int> un_use(n, false);
  for (auto [begin, end]: is.s) {
    if (is.is_inf(pair(begin, end))) {
      continue;
    }
