// detail: https://atcoder.jp/contests/arc128/submissions/26587721
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  auto dp = make_vector(n + 1, 2, 0.0);
  dp[0][0] = 1;

  rep (i, n) {
    int a = as[i];

    if (chmax(dp[i + 1][0], dp[i][0])) { bs[i + 1][0] = pair(i, 0); }
    if (chmax(dp[i + 1][0], dp[i][1] / a)) { bs[i + 1][0] = pair(i, 1); }

    if (chmax(dp[i + 1][1], dp[i][1])) { bs[i + 1][1] = pair(i, 1); }
    if (chmax(dp[i + 1][1], dp[i][0] * a)) { bs[i + 1][1] = pair(i, 0); }
  }
