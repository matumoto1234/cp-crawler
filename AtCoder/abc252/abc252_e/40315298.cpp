// detail: https://atcoder.jp/contests/abc252/submissions/40315298
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  map<UnWeightedEdge, int> edge_to_id;
  ...
  for (int i = 0; i < m; i++) {
    ...
    edge_to_id[UnWeightedEdge(a, b)] = i + 1;
    edge_to_id[UnWeightedEdge(b, a)] = i + 1;
  }
  ...
  for (int s = 1; s < n; s++) {
    ...
    while (to != -1) {
      ...
      edges.push_back(UnWeightedEdge(v, to));
