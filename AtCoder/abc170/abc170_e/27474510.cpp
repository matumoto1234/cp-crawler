// detail: https://atcoder.jp/contests/abc170/submissions/27474510
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  rep(i, n) {
    ...
    rates[i] = a;
    affiliations[i] = b;
    groups[b].emplace(rates[i], i);
  }

  map<int, int> equalities;
  ...
  while (q--) {
    ...
    int rate = rates[c];
    int grp = affiliations[c];

    Student s(rate, c);

    if (*groups[grp].rbegin() == s) {
      equalities[rate]--;
      if (equalities[rate] == 0) equalities.erase(rate);
      if (len(groups[grp]) >= 2) {
        int next_max_rate = next(groups[grp].rbegin())->first;
        equalities[next_max_rate]++;
      }
    }

    groups[grp].erase(s);

    int old_max_rate = -INF32;
    if (!groups[d].empty()) old_max_rate = groups[d].rbegin()->first;

    groups[d].emplace(s);

    if (old_max_rate == -INF32) {
      equalities[rate]++;
    } else if (*groups[d].rbegin() == s) {
      equalities[old_max_rate]--;
      if (equalities[old_max_rate] == 0) equalities.erase(old_max_rate);
      equalities[rate]++;
    }

    cout << equalities.begin()->first << endl;
  }
}