// detail: https://atcoder.jp/contests/abc088/submissions/13688275
#include <bits/stdc++.h>
using namespace std;

int h, w, cnt = 0, cntMin = INT_MAX, ret = INT_MAX;
vector<string> s(50);
vector<vector<int>> cntMap(50, vector<int>(50, INT_MAX));
...
int main() {
  ...
  for (int i = 0; i < h;i++){
    for (int j = 0; j < w;j++){
      cout << cntMap[i][j] << flush;
      j < w - 1 ? cout << " " << flush : cout << endl;
    }
  }*/
  return 0;
}