// detail: https://atcoder.jp/contests/abc001/submissions/27896340
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  IntervalSet<int> minutes;

  auto to_hour = [](int minute) -> pii {
    return pii(minute / 60, minute % 60);
  };

  auto to_minute = [](pii hour) -> int {
  ...
  rep(i, n) {
    ...
    rep(j, 2) {
      ...
      lr[j] = to_minute(pii(hour, minute));
    }
