// detail: https://atcoder.jp/contests/abc216/submissions/25430021
#include <bits/stdc++.h>
using namespace std;
#include <atcoder/scc>
...
int main() {
  ...
  range(i,m){
    range(j,len(as[i])-1){
      if(es.find(make_pair(as[i][j],as[i][j+1]))!=es.end()){
        continue;
      }
      es.emplace(as[i][j],as[i][j+1]);
