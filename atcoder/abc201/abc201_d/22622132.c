// detail: https://atcoder.jp/contests/abc201/submissions/22622132
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  rrange(i,0,h){
    rrange(j,0,w){
      ...
      if((i+j)%2==0){
        dp[i][j]=-INF32;
        if(i+1 < h) dp[i][j]=max(dp[i][j],dp[i+1][j]+inc[i+1][j]);
        if(j+1 < w) dp[i][j]=max(dp[i][j],dp[i][j+1]+inc[i][j+1]);
      }
      else if((i+j)%2==1){
        dp[i][j]=INF32;
        if(i+1 < h) dp[i][j]=min(dp[i][j],dp[i+1][j]+inc[i+1][j]);
        if(j+1 < w) dp[i][j]=min(dp[i][j],dp[i][j+1]+inc[i][j+1]);
      }
    }
  }
