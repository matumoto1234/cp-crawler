// detail: https://atcoder.jp/contests/abc196/submissions/23294374
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  int h,w,a,b;
  cin>>h>>w>>a>>b;
  auto dp=make_vector(h+1,w+1,a+10,b+10,1<<w,0);
  dp[0][0][0][0][0]=1;
  rep(i,h) rep(j,w) rep(k,a+1) rep(l,b+1) rep(S,1<<w) {
    // debug(i,j,k,l,S,dp[i][j][k][l][S]);
    if(S>>j&1){
      int nextS=S&~(1<<j);
      if(j+1<w){
        dp[i][j+1][k][l][nextS]+=dp[i][j][k][l][S];
      }else{
        dp[i+1][0][k][l][nextS]+=dp[i][j][k][l][S];
      }
      continue;
    }
    ...
    if(j+1<w && !(S&(1<<(j+1)))){
      int nextS=S|(1<<(j+1));
      dp[i][j+1][k+1][l][nextS]+=dp[i][j][k][l][S];
    }
    // 縦置き
    if(i+1<h){
      int nextS=S|(1<<j);
      if(j+1<w){
        ...
        dp[i+1][0][k+1][l][nextS]+=dp[i][j][k][l][S];
      }
    }
    // 1*1
    int nextS=S;
    if(j+1<w){
      dp[i][j+1][k][l+1][nextS]+=dp[i][j][k][l][S];
    }else{
      dp[i+1][0][k][l+1][nextS]+=dp[i][j][k][l][S];
    }
  }
  // rep(j,w+1) rep(k,a+1) rep(l,b+1) rep(S,1<<w){
  //   debug(h,j,k,l,S,dp[h][j][k][l][S]);
  // }
  cout<<dp[h][0][a][b][0]<<endl;
}