// detail: https://atcoder.jp/contests/yahoo-procon2018-final-open/submissions/30497504
#include <bits/stdc++.h>
using namespace std;
...
int find_index(const vector<int> &vs, int target) {
  rep(i, len(vs)) {
    if (vs[i] == target)
      return i;
  }
...
int main() {
  ...
  const vector<int> pre_primes = { 2, 3, 5, 7, 11, 13, 17 };
  ...
  rep(i, 1, m + 1) {
    ...
    vector<int> new_ps;
    for (auto p: ps) {
      int idx = find_index(pre_primes, p);
      if (idx == -1) {
        if (p > pre_primes.back()) {
          new_ps.push_back(p);
        }
