// detail: https://atcoder.jp/contests/abc217/submissions/25601051
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  range(l,n){
    int r=l+2;
    if(r>n) break;
    if(es.find(make_pair(l,r))==es.end()) continue;
    dp[l][r]=1;
    // range(r,l+2,n+1){
    //   if(es.find(make_pair(l,r))==es.end()) continue;
    //   dp[l][r]=1;
  ...
  range(i,1,n){
    range(l,n){
      ...
      if(l-2>=0){
        dp[l-2][r]+=dp[l-2][r-2]*dp[l][r];
      }
      if(r+2<=n){
        dp[l][r+2]+=dp[l][r]*dp[l+2][r+2];
      }
      // debug(l,r,dp[0][4].val());
    }
  }

  // range(i,n){
  //   range(j,n+1){
  //     debug(i,j,dp[i][j].val());
  //   }
  // }
