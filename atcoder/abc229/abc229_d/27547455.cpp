// detail: https://atcoder.jp/contests/abc229/submissions/27547455
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  char fuga = '.';
  char &hoge2 = fuga;
  if (vs.front().first == 'X') { vs.insert(vs.begin(), pair<char&, int>(hoge2, 0)); }
  if (vs.back().first == '.') { vs.emplace_back(hoge, 0); }
  ...
  rep(i, n) {
    ...
    auto [cost, val] = sum.query(i, valid);
    // debug(i, valid, cost, val, sum.query(i, i + 1).first);
    cost -= sum.query(i, i + 1).first;
    // debug(cost);
