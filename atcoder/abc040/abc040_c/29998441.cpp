// detail: https://atcoder.jp/contests/abc040/submissions/29998441
#include <bits/stdc++.h>
using namespace std;
...
vector<ll> dp(MAXN + 1, INF64);
ll dfs(int idx) {
  if (idx == 0)
    return 0;
  if (idx == 1)
    return abs(as[1] - as[0]);

  ll &res = dp[idx];
  if (res != INF64)
    return res;

  ll cost1 = dfs(idx - 1) + abs(as[idx] - as[idx - 1]);
  ll cost2 = dfs(idx - 2) + abs(as[idx] - as[idx - 2]);
  res = min(cost1, cost2);
  return res;
}
