// detail: https://atcoder.jp/contests/atc001/submissions/29930823
#include <bits/stdc++.h>
using namespace std;
...
pii get_start(const vector<string> &s) {
  rep(i, len(s)) {
    rep(j, len(s[i])) {
      if (s[i][j] == 's')
        return pii(i, j);
...
pii get_goal(const vector<string> &s) {
  rep(i, len(s)) {
    rep(j, len(s[i])) {
      if (s[i][j] == 'g')
        return pii(i, j);
...
int main() {
  ...
  pii start = get_start(s);

  stack<pii> st;
  st.push(start);
  auto used = make_vector(h, w, false);
