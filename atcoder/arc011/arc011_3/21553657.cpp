// detail: https://atcoder.jp/contests/arc011/submissions/21553657
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  G.build(distance(words.begin(),words.find(first)));
  ...
  vector<int> bs = G.restore(distance(words.begin(),words.find(last)));
  cout<<bs.size()-2<<endl;
  range(i,0,bs.size()){
    cout<<wds[bs[i]]<<'\n';
    // cout<<bs[i]<<endl;
  }
  if(wds[bs.back()]!=last) cout<<last<<endl;
}