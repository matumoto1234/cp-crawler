// detail: https://atcoder.jp/contests/arc119/submissions/22672668
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  vector<ll> odd=a,even=a;
  range(i,0,n) if(i%2==0) odd[i]*=-1;
  range(i,0,n) if(i%2==1) even[i]*=-1;

  //CumulativeSum<ll> oddsum(odd);
  CumulativeSum<ll> evensum(even);

  //CumulativeSum<ll> sum(a);
  //cout<<sum.sum<<endl;
  //cout<<oddsum.sum<<endl;
  //cout<<evensum.sum<<endl;
  map<ll,ll> cntmp;
  range(i,0,n+1){
    cntmp[evensum.sum[i]]++;
  }
