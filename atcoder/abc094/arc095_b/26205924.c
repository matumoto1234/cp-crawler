// detail: https://atcoder.jp/contests/abc094/submissions/26205924
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  int idx=lower_bound(as.begin(),as.end(),mid)-as.begin();

  int dist=INF32;
  int ans=-1;
  if(idx < len(as) and chmin(dist,abs(mid-as[idx]))){
    ans=idx;
  }

  if(idx-1>=0 and chmin(dist,abs(mid-as[idx-1]))){
    ans=idx-1;
  }

  if(idx+1<len(as) and chmin(dist,abs(mid-as[idx+1]))){
    ans=idx+1;
  }

  cout<<back<<' '<<as[ans]<<endl;
}