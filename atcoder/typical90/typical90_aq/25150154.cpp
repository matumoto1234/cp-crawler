// detail: https://atcoder.jp/contests/typical90/submissions/25150154
#include <bits/stdc++.h>
using namespace std;
  ...
  while ( !q.empty() ) {
    ...
    for ( int i = 0; i < 4; i++ ) {
      for(int j=0;j<4;j++){
        ...
        if(i==j){
          if(chmin(res[ny][nx][j],res[p.first][p.second][i])){
            q.emplace_front(ny, nx);
          }
        }else{
          if(chmin(res[ny][nx][j],res[p.first][p.second][i] + 1)){
