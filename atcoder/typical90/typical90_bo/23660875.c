// detail: https://atcoder.jp/contests/typical90/submissions/23660875
#include <bits/stdc++.h>
using namespace std;
...
ll to_decimal(vector<ll> n, ll b){
	ll res=0;
	for(ll i=n.size()-1,x=1;i>=0;i--,x*=b){
		res+=x*(n[i]);
	}
	return res;
}
...
vector<ll> convert_base(vector<ll> n,ll from,ll to){
	ll temp=to_decimal(n,from);
...
int main() {
	...
	auto vs=to_base(n,10);
	rep(i,k){
		vs=convert_base(vs,8,9);
		for(auto &v:vs){
			if(v==8) v=5;
		}
	}
