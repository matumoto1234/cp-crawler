// detail: https://atcoder.jp/contests/abc238/submissions/30522495
#include <bits/stdc++.h>
using namespace std;
...
bool solve(ll a, ll s) {
  ...
  rep(i, 60) {
    if (bit_a[i] == 0 and bit_s[i] == 0 and not carry) {
      continue;
    }
    if (bit_a[i] == 0 and bit_s[i] == 0 and carry) {
      continue;
    }

    if (bit_a[i] == 0 and bit_s[i] == 1 and not carry) {
      continue;
    }
    if (bit_a[i] == 0 and bit_s[i] == 1 and carry) {
      carry = false;
      continue;
    }

    if (bit_a[i] == 1 and bit_s[i] == 0 and not carry) {
      carry = true;
      continue;
    }
    if (bit_a[i] == 1 and bit_s[i] == 0 and carry) {
      return false;
    }

    if (bit_a[i] == 1 and bit_s[i] == 1 and not carry) {
      return false;
    }
    if (bit_a[i] == 1 and bit_s[i] == 1 and carry) {
      carry = true;
      continue;
    }
  }
