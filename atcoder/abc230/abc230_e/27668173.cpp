// detail: https://atcoder.jp/contests/abc230/submissions/27668173
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  for (ll i = 1; i * i < n; i++) {
    ...
    vals.emplace_back(valid, i);
  }

  // make unique point
  vector<pll> unique_points;
  unique_points.reserve(vals.size());

  for (const auto &[point, val]: vals) {
    if (unique_points.empty()) {
      unique_points.emplace_back(point, val);
      continue;
    }

    const auto &[p, v] = unique_points.back();
    if (point == p) continue;

    unique_points.emplace_back(point, val);
  }

  rwhole(sort, unique_points);

  ll pre = n + 1;
  for (const auto &[p, v]: unique_points) {
    ans += (pre - p) * v;
    pre = p;
  }
