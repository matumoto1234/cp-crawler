// detail: https://atcoder.jp/contests/abc248/submissions/31022727
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  rep(_, q) {
    ...
    if (r <= vals.at(x).front()) {
      cout << 0 << newl;
      continue;
    }

    int lower = whole(lower_bound, vals.at(x), l) - begin(vals.at(x));
    int upper = whole(upper_bound, vals.at(x), r) - begin(vals.at(x));
