// detail: https://atcoder.jp/contests/code-festival-2014-qualb/submissions/26553485
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  for (auto [key, val]: cnt3) {
    if (cnt1[key] + cnt2[key] < val) ans = false;
    if (val >= n) {
      if (cnt1[key] <= n and cnt2[key] <= n) { continue; }
      ans = false;
    }
  }
